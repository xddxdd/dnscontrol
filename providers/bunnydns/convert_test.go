...
func TestFromRecordConfigScript(t *testing.T) {
	...
	if rec.ScriptID != 12345 {
		t.Fatalf("expected ScriptId=12345; got=%d", rec.ScriptID)
	}
	if rec.Value != "" {
...
func TestFromRecordConfigScriptInvalidTarget(t *testing.T) {
	rc := &models.RecordConfig{
		Type: "BUNNY_DNS_SCRIPT",
	}
	rc.SetLabelFromFQDN("cdn.example.com", "example.com")
...
func TestToRecordConfigScriptLinkName(t *testing.T) {
	rec := &record{
